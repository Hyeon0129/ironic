...
5. **Configdrive JSON 주입 방식:** Cloud-init User-Data는 JSON 포맷(`{"user_data": "..."}`)으로 Ironic API를 통해 주입. 네트워크/메타데이터 이슈 발생 시 이 부분을 우선 점검.
...
7. **백엔드 프로세스 관리 제한:** 8000번 포트는 사용자가 상시 모니터링/사용 중이므로, 코드 수정 후에도 `pkill`, `kill` 등으로 프로세스를 강제 종료하거나 서버를 재시작하지 않는다. 서비스 반영은 사용자가 직접 관리.
